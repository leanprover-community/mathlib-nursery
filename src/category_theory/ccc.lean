...
{ unit := augment x,
  ...
  left_id :=
    begin
      ...
      { rw [← this], simp only [id_comp], },
      rw [← left_cancel'], simp only [Hone,comp_id,iso.hom_inv_id,category.assoc],
    end,
  right_id :=
    begin
      generalize Hone : 𝟙 x = one,
      ...
      { rw [← this], simp only [id_comp], },
      rw ← right_cancel', simp only [Hone,comp_id,iso.hom_inv_id,category.assoc],
    end,
  assoc := sorry }
...
{ diagonal := λ X,  (diagonal X.fst, diagonal X.snd),
  ...
  left_cancel'  := begin intros x, tactic.ext1 [] {new_goals := tactic.new_goals.all},
                         { cases x, dsimp at *, apply left_cancel' },
                         { cases x, dsimp at *, apply left_cancel' },
                   end }
...
{ obj := λ d, h.obj (c,d),
  ...
  map_id' := by { intros, erw functor.map_id, },
  map_comp' := by { intros, rw [← functor.map_comp], simp only [(≫), comp_id, eq_self_iff_true] } }
...
{ obj := λ f, ((CCC.hom C₀).obj (f.1.1,f.2.1),(CCC.hom C₁).obj (f.1.2,f.2.2)),
  map := λ (X Y : (C₀ × C₁)ᵒᵖ × C₀ × C₁) (f : (X.1 ⟶ Y.1) × (X.2 ⟶ Y.2)),
    ((hom.{u v} C₀).map (f.1.1,f.2.1), (hom.{u v} C₁).map (f.1.2,f.2.2)),
  map_id' := by { intros, erw [functor.map_id,functor.map_id], refl },
  map_comp' := by { intros, dsimp [(≫)], erw [← functor.map_comp,← functor.map_comp], refl, } }

def prod.i : functor.id.{u v} (C₀ × C₁) ≅ curry.{u v} (prod.hom.{u v} C₀ C₁) (I.{u v} (C₀ × C₁)) :=
{ hom := { app := λ (X : C₀ × C₁), ((i.{u v} C₀).hom.app X.1, ((i C₁).hom.app X.2 )),
           naturality' := by { intros, simp only [(≫), curry, id_map],
                               erw [← nat_trans.naturality,← nat_trans.naturality], split; refl } },
