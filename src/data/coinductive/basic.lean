...
Work in progress
-/
import data.pfun.nursery tactic.nursery

universes u v w

open nat function roption list (hiding head')
...
@[simp]
lemma cases_on_mk {r : cofix β → Sort*} (x : α) (ch : β x → cofix β) (f : Π x (ch : β x → cofix β), r (cofix.mk x ch))
: cofix.cases_on (cofix.mk x ch) f = f x ch :=
cases_mk x ch f
