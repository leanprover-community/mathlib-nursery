...
-- protected def bind {α β} (a : computation α) (f : α → computation β) : computation β :=
-- proxy_t.corec
-- (λ x : proxy_t i i' o o' α,
-- match unroll x with
-- | (proxy_t.f.pure x) := sum.inl (f x)
-- | (proxy_t.f.write x f) := sum.inr $ proxy_t.f.write x f
-- | (proxy_t.f.read x f) := sum.inr $ proxy_t.f.read x f
-- end )
-- a
